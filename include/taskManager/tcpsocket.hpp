...
public:
    ...
    int SendJson(const std::string& jsonstring) {  // json-over-tcp 라이브러리를 사용하기 위한 프로토콜 값 추가 전송
        uint16_t protocol_signature = 206;
        uint32_t message_length = static_cast<uint32_t>(jsonstring.size());
        Send(reinterpret_cast<const char*>(&protocol_signature), sizeof(protocol_signature));
        Send(reinterpret_cast<const char*>(&message_length), sizeof(message_length));
        Send(jsonstring);
    }
